...
class PRStackViewTestCase: XCTestCase {
    
    var uiStackView: UIStackView!
    var tzStackView: PRStackView!
    ...
    override func setUp() {
        ...
        tzStackView = PRStackView(arrangedSubviews: createTestViews())
    ...
    func logAllConstraints() {
        ...
        print("================= PRSTACKVIEW (\(tzStackView.constraints.count)) =================")
    ...
    // To override in subclass
    func configureStackViews(_ uiStackView: UIStackView, _ tzStackView: PRStackView) {
    }
    ...
    fileprivate func hasSameConstraintMetaData(_ layoutConstraint1: NSLayoutConstraint, _ layoutConstraint2: NSLayoutConstraint) -> Bool {
        ...
        if layoutConstraint1.priority != layoutConstraint2.priority {
            if Int(layoutConstraint1.priority.rawValue) < 100 || Int(layoutConstraint1.priority.rawValue) > 150
                || Int(layoutConstraint2.priority.rawValue) < 100 || Int(layoutConstraint2.priority.rawValue) > 150 {
                    return false
            }
        }
        return true
    ...
    fileprivate func viewsEqual(_ object1: AnyObject?, _ object2: AnyObject?) -> Bool {
        ...
        if object1 is UIStackView && object2 is PRStackView {
            return true
        }
        if object1 is PRStackView && object2 is UIStackView {
            return true
        }
        // Wish I could assert more accurately than this
        if object1 is UILayoutGuide && object2 is PRSpacerView {
            return true
        }
        // Wish I could assert more accurately than this
        if object1 is PRSpacerView && object2 is UILayoutGuide {
            return true
        }
        return false
    }
