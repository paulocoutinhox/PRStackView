...
struct PRAnimationDidStopQueueEntry: Equatable {
    let view: UIView
    let hidden: Bool
}

func ==(lhs: PRAnimationDidStopQueueEntry, rhs: PRAnimationDidStopQueueEntry) -> Bool {
    return lhs.view === rhs.view
}

public class PRStackView: UIView {

    public var distribution: PRStackViewDistribution = .fill {
        didSet {
    ...
    public var alignment: PRStackViewAlignment = .fill
    ...
    private var animationDidStopQueueEntries = [PRAnimationDidStopQueueEntry]()
    ...
    public override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if let view = object as? UIView, let change = change, keyPath == "hidden" {
            ...
            if let _ = view.layer.animationKeys() {
                ...
                animationDidStopQueueEntries.insert(PRAnimationDidStopQueueEntry(view: view, hidden: hidden), at: 0)
                UIView.setAnimationDidStop(#selector(PRStackView.hiddenAnimationStopped))
            } else {
    ...
    @objc func hiddenAnimationStopped() {
        var queueEntriesToRemove = [PRAnimationDidStopQueueEntry]()
    ...
    private func addSpacerView() -> PRSpacerView {
        let spacerView = PRSpacerView()
